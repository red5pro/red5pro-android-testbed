...
/**
 * This sample demonstrates voice only conferencing
 */
class VoiceOnlyConferenceActivity : AppCompatActivity(), IRed5WebrtcClient.Red5EventListener {
    ...
    private lateinit var localAudioSection: RelativeLayout
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        setContentView(R.layout.activity_voice_only_conference)
        ...
        conferenceListener = object : IRed5WebrtcClient.ConferenceListener {
            ...
            override fun onJoinRoomFailed(statusCode: Int, message: String?) {
                Toast.makeText(
                    this@VoiceOnlyConferenceActivity,
    ...
    private fun initViews() {
        ...
        localAudioSection = findViewById(R.id.localAudioSection)
    ...
    private fun addParticipant(
        ...
        videoEnabled: Boolean,
        audioEnabled: Boolean,
        renderer: Any?  // Not used for voice-only
    ) {
        // Use simple subscriber layout for voice-only (no video renderer needed)
        val participantView = layoutInflater.inflate(
            R.layout.item_voice_only_participant,
            participantsGridLayout,
            false
        )

        val userIdText: TextView = participantView.findViewById(R.id.voiceOnlyParticipantUserId)
        val micIcon: ImageView = participantView.findViewById(R.id.voiceOnlyParticipantMicIcon)
        ...
        val holder = ParticipantViewHolder(
            participantView,
            null, // No renderer for voice-only
            userIdText,
            micIcon,
            null  // No camera layout for voice-only
        )
    ...
    fun initSdk() {
        val builder = IRed5WebrtcClient.builder()
            ...
            .setToken("")
            .setVideoEnabled(false)  // Voice-only - no video
            .setEventListener(this)
            .setConferenceListener(conferenceListener)

        // Only enable audio for publishers
    ...
    private fun fetchTokenAndJoin(userId: String, metaData: String) {
        val host = SettingsActivity.Companion.getStreamManagerHost(this)
    ...
    private fun showConferenceLayout() {
        runOnUiThread {
            ...
            if (userRole == "subscriber") {
                ...
                localAudioSection.visibility = View.GONE
                ...
                localAudioSection.visibility = View.VISIBLE
                ...
                // Participants container below local audio section
                participantsParams.removeRule(RelativeLayout.BELOW)
                participantsParams.addRule(RelativeLayout.BELOW, R.id.localAudioSection)
    ...
    override fun onLicenseValidated(validated: Boolean, message: String?) {
        // No preview needed for voice-only conference
    }
    ...
    data class ParticipantViewHolder(
        val itemView: View,
        val renderer: Any?,  // Not used for voice-only
        val userIdText: TextView,
        val micIcon: ImageView?,
        val cameraOffLayout: RelativeLayout?  // Not used for voice-only
    )
}